...
<body>
  <div class="panel admin-panel">
    ...
    <div class="padding border-bottom">
      <ul>
      <li>
    	<form method="post" class="" action="<%=basePath%>queryUserByKey">
			<div id="">
	...
	<form method="post" action="<%=basePath%>deleteAllUser">
    <table class="table table-hover text-center">
      ...
      <c:forEach items="${data.items }" var="item" varStatus="status">
      	<tr>
          <td><input type="checkbox" class="checkboxId" name="id${status.index }" value="${item.id }" />
            ${status.index+1 }</td>
          <td>${item.name }</td>
          <td>
          ...
          <div class="button-group">
	          <c:if test="${item.status==1 }">
	          	<a type="button" class="button border-main" href="<%=basePath%>alterUserStatus?id=${item.id }&status=0"><span class="icon-edit"></span>冻结账号</a>
	          </c:if>
	          <c:if test="${item.status==0 }">
	          	<a type="button" class="button border-main" href="<%=basePath%>alterUserStatus?id=${item.id }&status=1"><span class="icon-edit"></span>解冻账号</a>
	          </c:if>
      ...
      <c:forEach items="${users }" var="item" varStatus="status">
		  ...
		  <td>
	          <div class="button-group">
		          <c:if test="${item.status==1 }">
		          	<a type="button" class="button border-main" href="<%=basePath%>alterUserStatus?id=${item.id }&status=0&page=${data.currentPage}"><span class="icon-edit"></span>冻结账号</a>
		          </c:if>
		          <c:if test="${item.status==0 }">
		          	<a type="button" class="button border-main" href="<%=basePath%>alterUserStatus?id=${item.id }&status=1&page=${data.currentPage}"><span class="icon-edit"></span>解冻账号</a>
		          </c:if>
		          <a class="button border-red" href="javascript:void(0)" onclick="return del(${item.id})"><span class="icon-trash-o"></span> 删除</a>
	          </div>
          </td>
        	...
        	<div class="pagelist">
        		<c:choose>
        			<c:when test="${data.currentPage>1 }">
						<a href="<%=basePath%>queryAllUser?page=${data.currentPage-1 }">上一页</a>
					</c:when>
					<c:otherwise>
				...
				<c:forEach var="page" begin="1" end="${totalPage }">
					...
					<c:if test="${page != data.currentPage }">
						<a href="<%=basePath%>queryAllUser?page=${page }">${page }</a>
					</c:if>
				</c:forEach>
				<c:choose>
					<c:when test="${data.currentPage<totalPage }">
						<a href="<%=basePath%>queryAllUser?page=${data.currentPage+1 }">下一页</a>
					</c:when>
					<c:otherwise>
